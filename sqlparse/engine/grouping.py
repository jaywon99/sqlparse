...
def group_comparison(tlist):
    ...
    def match(token):
        return token.ttype == T.Operator.Comparison or \
            (token.is_keyword and token.normalized == 'IS')

    def valid(token):
        if imt(token, t=ttypes, i=sqlcls):
            return True
        elif token and token.is_keyword and token.normalized in ('NULL', 'NOT NULL'):
            return True
        else:
            return False
...
def group_between_prior(tlist):
    sqlcls = (sql.Parenthesis, sql.Function, sql.Identifier,
              sql.Operation, sql.TypedLiteral)
    ttypes = T_NUMERICAL + T_STRING + T_NAME

    def match(token):
        return token.is_keyword and token.normalized == 'BETWEEN'
    ...
    valid_prev = valid_next = valid
    _group(tlist, sql.Between, match,
           valid_prev, valid_next, post, extend=False)


def group_between_next(tlist):
    sqlcls = (sql.Parenthesis, sql.Function, sql.Identifier,
              sql.Operation, sql.TypedLiteral)
    ttypes = T_NUMERICAL + T_STRING + T_NAME

    def match(token):
        return token.is_keyword and token.normalized == 'AND'

    def valid_prev(token):
        if imt(token, i=(sql.Between)):
            return True
...
def group_identifier(tlist):
    ...
    # somebody using date as field name.. :(
    ttypes = (T.String.Symbol, T.Name, T.Name.Builtin)
...
def group_on(tlist):
    ...
    while token:
        ...
        tidx, token = tlist.token_next_by(m=sql.On.M_OPEN, idx=tidx)
